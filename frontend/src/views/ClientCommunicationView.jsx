      ...
      <div className="flex gap-4 flex-1 min-h-0">
        ...
        <div className="w-72 shrink-0 bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm flex flex-col overflow-hidden">
          <div className="px-4 py-3 bg-gray-50 dark:bg-slate-900 border-b border-gray-200 dark:border-slate-700 shrink-0">
            <p className="text-xs font-bold text-gray-500 dark:text-slate-400 uppercase tracking-wider">Active Tickets</p>
          </div>
          <div className="flex-1 overflow-y-auto divide-y divide-gray-100">
            {ticketList.length === 0 ? (
              <p className="text-xs text-gray-400 dark:text-slate-500 text-center py-8">No tickets available</p>
            ) : (
              ticketList.map(t => {
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left px-4 py-3.5 transition-colors
                      ${selected?.id === t.id ? 'bg-blue-50 border-l-2 border-blue-600' : 'hover:bg-gray-50 dark:bg-slate-900 border-l-2 border-transparent'}`}
                  >
                    <div className="flex items-start justify-between gap-1 mb-1">
                      <span className="text-[10px] font-bold text-blue-600">#{t.id.slice(0,8).toUpperCase()}</span>
                      <span className={`text-[9px] font-semibold px-1.5 py-0.5 rounded-full
                        ${t.priority === 'Critical' ? 'bg-red-100 text-red-700' :
                          t.priority === 'High' ? 'bg-orange-100 text-orange-700' : 'bg-gray-100 dark:bg-slate-700 text-gray-500 dark:text-slate-400'}`}>
                        {t.priority}
                      </span>
                    </div>
                    <p className="text-xs font-semibold text-gray-800 truncate mb-1">{t.title}</p>
                    <p className="text-[10px] text-gray-400 dark:text-slate-500">{t.client?.full_name || '—'}</p>
                  </button>
                );
              })
            )}
        ...
        {/* Communication panel */}
        {!detail ? (
          <div className="flex-1 bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm flex flex-col items-center justify-center text-gray-400 dark:text-slate-500 gap-3">
          ...
          <div className="flex-1 bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm flex flex-col overflow-hidden">
            ...
            {/* Ticket header */}
            <div className="px-5 py-4 border-b border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-900 shrink-0">
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <h3 className="text-sm font-bold text-gray-900 dark:text-white">{detail.title}</h3>
                </div>
            ...
            {/* Tabs */}
            <div className="flex border-b border-gray-200 dark:border-slate-700 px-5 shrink-0">
              {[
                ...
                <button
                  ...
                  className={`flex items-center gap-1.5 px-4 py-3 text-xs font-semibold border-b-2 -mb-px transition-colors
                    ${tab === t.key ? 'border-blue-600 text-blue-600' : 'border-transparent text-gray-500 dark:text-slate-400 hover:text-gray-700 dark:text-slate-200'}`}
                >
                  <span>{t.icon}</span>
                  {t.label}
            ...
            <div className="flex-1 overflow-y-auto px-5 py-4 flex flex-col gap-3 min-h-0">
              {displayed.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-32 text-gray-400 dark:text-slate-500 gap-2">
                  ...
                  return (
                    <div key={c.comment_id} className={`flex items-end gap-2.5 ${isAgent ? 'flex-row-reverse' : 'flex-row'}`}>
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center text-[11px] font-bold shrink-0
                        ${isAgent ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700 dark:text-slate-200'}`}>
                        {initials(c.author?.full_name)}
                      </div>
                      <div className={`flex flex-col max-w-[70%] gap-1 ${isAgent ? 'items-end' : 'items-start'}`}>
                        <p className="text-[10px] text-gray-400 dark:text-slate-500 px-1">{c.author?.full_name} · {c.author?.role}</p>
                        <div className={`px-4 py-2.5 rounded-2xl text-sm leading-relaxed
                          ${isAgent
                            ? 'bg-blue-600 text-white rounded-br-sm'
                            : 'bg-gray-100 dark:bg-slate-700 text-gray-800 rounded-bl-sm'
                          }
            ...
            {/* Reply box */}
            <div className="border-t border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-900 px-5 py-4 shrink-0">
              <textarea
                className="w-full border border-gray-200 dark:border-slate-700 rounded-xl px-4 py-3 text-sm bg-white dark:bg-slate-800 outline-none focus:border-blue-400 resize-none placeholder-gray-400 transition-all"
