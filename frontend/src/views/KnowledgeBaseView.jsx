        ...
        <div className="w-72 shrink-0 bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm flex flex-col overflow-hidden">
          ...
          <div className="px-4 py-3 border-b border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-900 shrink-0">
            <div className="flex gap-2">
              <input
                className="flex-1 px-3 py-2 border border-gray-200 dark:border-slate-700 rounded-lg text-xs bg-white dark:bg-slate-800 outline-none focus:border-blue-400 placeholder-gray-400"
              ...
              articles.map(a => (
                <button
                  ...
                  className={`w-full text-left px-4 py-3.5 transition-colors
                    ${selected?.article_id === a.article_id ? 'bg-blue-50 border-l-2 border-blue-600' : 'hover:bg-gray-50 dark:bg-slate-900 border-l-2 border-transparent'}`}
                >
                  <div className="flex items-start gap-2">
                    <span className="text-sm shrink-0 mt-0.5">??</span>
                    <div className="min-w-0">
                      <p className="text-xs font-semibold text-gray-800 truncate">{a.title}</p>
                      <p className="text-[10px] text-gray-400 dark:text-slate-500 mt-0.5">KB-{a.article_id} · {a.author?.full_name || 'Unknown'}</p>
                    </div>
                  </div>
                </button>
              ))
          ...
          {creating ? (
            <div className="flex flex-col gap-4 p-6 overflow-y-auto">
              <div className="flex items-center justify-between">
                ...
                <button onClick={() => setCreating(false)} className="text-gray-400 dark:text-slate-500 hover:text-gray-600 dark:text-slate-300 text-xs">? Cancel</button>
              </div>
              <input
                className="px-4 py-3 border border-gray-200 dark:border-slate-700 rounded-xl text-sm font-medium outline-none focus:border-blue-400 placeholder-gray-400"
                placeholder="Article title..."
                value={form.title}
                onChange={e => setForm(f => ({ ...f, title: e.target.value }))}
              />
              <textarea
                className="px-4 py-3 border border-gray-200 dark:border-slate-700 rounded-xl text-sm outline-none focus:border-blue-400 resize-none leading-relaxed placeholder-gray-400"
              ...
              <div className="flex gap-3 justify-end">
                <button onClick={() => setCreating(false)} className="px-5 py-2.5 bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-slate-300 rounded-xl text-sm font-medium hover:bg-gray-200 transition-colors">Cancel</button>
            ...
            <div className="flex flex-col h-full overflow-hidden">
              {/* Article header */}
              <div className="px-6 py-5 border-b border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-900 shrink-0">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <p className="text-[10px] font-bold text-gray-400 dark:text-slate-500 uppercase tracking-wider mb-1">KB-{selected.article_id}</p>
              ...
              <div className="flex-1 overflow-y-auto px-6 py-5">
                <div className="prose prose-sm max-w-none text-gray-700 dark:text-slate-200 leading-8 whitespace-pre-wrap text-sm">{selected.content}</div>
              </div>
            </div>
          ) : (
