...
export default function TicketQueueView({ onOpenTicket }) {
  ...
  const kpiCards = [
    ...
    { label: 'SLA Due Today',    value: stats?.slaBreaches ?? '—',   bg: (stats?.slaBreaches ?? 0) > 0 ? 'bg-red-50' : 'bg-gray-50', border: (stats?.slaBreaches ?? 0) > 0 ? 'border-red-300' : 'border-gray-200', text: (stats?.slaBreaches ?? 0) > 0 ? 'text-red-700' : 'text-gray-700' },
  ];
  ...
  return (
    <div className="flex flex-col h-full gap-5">
      ...
      <div>
        ...
        <p className="text-sm text-gray-500 mt-0.5">Monitor and manage all incoming support tickets</p>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden flex-1 flex flex-col min-h-0">
          ...
          <div className="flex gap-1 border-b border-gray-200 w-full">
            ...
            <div className="ml-auto mb-1 flex items-center gap-2">
              <div className="relative">
                ...
                <input
                  className="pl-7 pr-3 py-1.5 bg-gray-50 border border-gray-200 rounded-lg text-xs outline-none focus:border-blue-400 focus:bg-white transition-all w-52 placeholder-gray-400"
                  placeholder="Search tickets..."
                  value={search}
                  onChange={e => { setSearch(e.target.value); setPage(1); }}
                />
              </div>
              <button onClick={load} className="px-2.5 py-1.5 border border-gray-200 rounded-lg text-xs text-gray-500 hover:bg-gray-50 hover:text-blue-600 transition-colors">↻</button>
            </div>
          </div>
        </div>

        {/* Table header */}
        <div className="grid grid-cols-[1fr_160px_100px_130px_120px] px-5 py-2.5 text-[11px] font-bold text-gray-400 uppercase tracking-wider border-b border-gray-100 bg-gray-50 shrink-0">
          <span>Ticket</span>
          <span>Client</span>
          <span>Priority</span>
          <span>Status</span>
            ...
            tickets.map(ticket => {
              ...
              return (
                <div
                  ...
                  className="grid grid-cols-[1fr_160px_100px_130px_120px] items-center px-5 py-3.5 border-b border-gray-100 cursor-pointer hover:bg-blue-50/50 transition-colors group"
                >
                  {/* Ticket info */}
                  <div className="flex flex-col gap-0.5 min-w-0 pr-4">
                    ...
                    <span className="text-sm font-semibold text-gray-800 truncate group-hover:text-blue-700 transition-colors">{ticket.title}</span>
                    {ticket.category?.category_name && (
                      <span className="text-[10px] text-gray-400">{ticket.category.category_name}</span>
                    )}
                  </div>
                  ...
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    <span className="text-xs text-gray-600 truncate">{ticket.client?.full_name || '—'}</span>
                  </div>
        ...
        <div className="flex items-center justify-between px-5 py-3 border-t border-gray-200 bg-gray-50 shrink-0">
          ...
          <div className="flex items-center gap-2">
            <button disabled={page <= 1} onClick={() => setPage(p => p - 1)} className="px-3 py-1.5 border border-gray-200 rounded-lg bg-white text-xs text-gray-600 hover:bg-blue-50 hover:border-blue-300 disabled:opacity-40 disabled:cursor-not-allowed transition-all">← Prev</button>
            <span className="text-xs text-gray-500 font-medium px-1">{page} / {totalPages}</span>
            <button disabled={page >= totalPages} onClick={() => setPage(p => p + 1)} className="px-3 py-1.5 border border-gray-200 rounded-lg bg-white text-xs text-gray-600 hover:bg-blue-50 hover:border-blue-300 disabled:opacity-40 disabled:cursor-not-allowed transition-all">Next →</button>
          </div>
        </div>
      </div>
    </div>
