...
const ROLE_DASHBOARDS = {
  ...
  Developer:      '/dashboard/support', // temporary until dev dashboard is built
  Admin:          '/dashboard/support', // temporary until admin dashboard is built
  Client:         '/dashboard/support', // temporary until client portal is built
};
...
export default function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      login(token, user);
      const dest = location.state?.from?.pathname || ROLE_DASHBOARDS[user.role] || '/';
      ...
      <div className="flex-1 flex items-center justify-center px-4 py-12">
        <div className="w-full max-w-4xl bg-white rounded-2xl shadow-xl border border-gray-200 overflow-hidden flex">
          ...
          <div className="flex-1 px-10 py-12">
            ...
            <button className="w-full py-3 border border-gray-200 rounded-xl flex items-center justify-center gap-2.5 text-sm font-semibold text-gray-700 hover:bg-gray-50 transition-colors">
            ...
            <p className="text-center text-sm text-gray-500 mt-6">
              ...
              <Link to="/signup" className="text-blue-600 font-semibold hover:text-blue-700 transition-colors">Create an account</Link>
            </p>
          </div>
          ...
          <div className="w-80 bg-[#1a2332] px-8 py-12 flex flex-col justify-between shrink-0">
            <div>
              ...
              <p className="text-white/50 text-sm leading-relaxed">Join 500+ enterprise teams managing incidents with pixel-perfect precision.</p>
            </div>
                ...
                <div className="flex flex-col gap-2">
                  {['Ticket #4402 resolved by Sarah','New Incident #4405 reported'].map((a,i) => (
                    <div key={i} className="flex items-center gap-2.5">
                      <div className="w-7 h-7 bg-blue-600 rounded-full flex items-center justify-center text-white text-[9px] font-bold shrink-0">{i===0?'S':'A'}</div>
