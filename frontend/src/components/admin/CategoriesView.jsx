...
export default function CategoriesView() {
  ...
  return (
    <div className="flex flex-col gap-4">
      <div>
        <h2 className="text-lg font-bold text-gray-900 dark:text-white">Ticket Categories</h2>
        <p className="text-sm text-gray-500 dark:text-slate-400">Add, rename, or remove ticket classification categories</p>
      </div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm overflow-hidden">
        <div className="px-5 py-3 border-b border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-900 flex items-center justify-between">
          <span className="text-xs font-bold text-gray-500 dark:text-slate-400 uppercase tracking-wider">Categories ({cats.length})</span>
        </div>
        {loading ? (
          ...
          <div className="flex flex-col items-center py-12 text-gray-400 dark:text-slate-500 gap-2"><span className="text-3xl">??</span><p className="text-sm">No categories yet — add your first one above</p></div>
        ) : (
          cats.map(c => (
