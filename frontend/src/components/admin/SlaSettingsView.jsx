...
export default function SlaSettingsView() {
  ...
  return (
    <div className="flex flex-col gap-4">
      <div>
        <h2 className="text-lg font-bold text-gray-900">SLA Settings</h2>
        <p className="text-sm text-gray-500">Configure response and resolution time targets. Changes take effect immediately across all active tickets.</p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        {loading
          ...
          : rules.map(rule => {
              const s = PRIORITY_STYLE[rule.priority] || { badge: 'bg-gray-100 text-gray-600', accent: 'border-l-gray-400' };
              const isEditing = editing?.sla_id === rule.sla_id;
              return (
                <div key={rule.sla_id} className={`bg-white rounded-xl border border-gray-200 border-l-4 ${s.accent} shadow-sm p-5`}>
                  <div className="flex items-center justify-between mb-4">
                    <span className={`text-sm font-bold px-3 py-1.5 rounded-lg ${s.badge}`}>{rule.priority} Priority</span>
                    {!isEditing ? (
                      <button onClick={() => setEditing({ ...rule })} className="px-3 py-1.5 text-xs font-semibold border border-gray-200 rounded-lg hover:bg-blue-50 hover:border-blue-300 hover:text-blue-600 transition-all">Edit</button>
                    ) : (
                      <div className="flex gap-2">
                        <button onClick={() => setEditing(null)} className="px-3 py-1.5 text-xs border border-gray-200 rounded-lg hover:bg-gray-100">Cancel</button>
