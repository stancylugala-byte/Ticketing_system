...
export default function TicketDistributionView() {
  ...
  return (
    <div className="flex flex-col gap-4">
      <div>
        <h2 className="text-lg font-bold text-gray-900 dark:text-white">Ticket Distribution</h2>
        <p className="text-sm text-gray-500 dark:text-slate-400">Breakdown of tickets by current lifecycle status</p>
      </div>

      <div className="grid grid-cols-2 gap-4">
        {/* Bar chart */}
        <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm p-6">
          <h3 className="text-sm font-bold text-gray-700 dark:text-slate-200 mb-5">Status Breakdown</h3>
          {loading ? (
            <div className="flex flex-col gap-3">{[...Array(5)].map((_,i) => <div key={i} className="h-10 bg-gray-100 dark:bg-slate-700 rounded animate-pulse" />)}</div>
          ) : (
            <div className="flex flex-col gap-4">
              {data.map(d => {
                const pct = total > 0 ? (parseInt(d.count) / total) * 100 : 0;
                const c   = STATUS_COLORS[d.status] || { bar: 'bg-gray-400', text: 'text-gray-600 dark:text-slate-300', bg: 'bg-gray-50 dark:bg-slate-900' };
                return (
                  <div key={d.status}>
                    <div className="flex justify-between items-center mb-1.5">
                      <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${c.bg} ${c.text}`}>{d.status}</span>
                      <span className="text-sm font-bold text-gray-800 dark:text-slate-100">{d.count} <span className="text-xs font-normal text-gray-400 dark:text-slate-500">({pct.toFixed(1)}%)</span></span>
                    </div>
        ...
        <div className="flex flex-col gap-3">
          {data.map(d => {
            ...
            const c   = STATUS_COLORS[d.status] || { bar: 'bg-gray-400', text: 'text-gray-600 dark:text-slate-300', bg: 'bg-gray-50 dark:bg-slate-900' };
            return (
              <div key={d.status} className={`bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm px-5 py-4 flex items-center justify-between`}>
                <div className="flex items-center gap-3">
                  <span className={`w-3 h-3 rounded-full ${c.bar}`} />
                  <span className="text-sm font-semibold text-gray-700 dark:text-slate-200">{d.status}</span>
                </div>
                <div className="text-right">
                  <p className="text-xl font-bold text-gray-900 dark:text-white">{d.count}</p>
                  <p className="text-xs text-gray-400 dark:text-slate-500">{pct}% of total</p>
                </div>
              </div>
            );
          })}
