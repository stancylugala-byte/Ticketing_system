    ...
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">High and Critical priority tickets requiring immediate attention</p>
        </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        <div className="grid grid-cols-[1fr_130px_90px_110px_120px] px-5 py-2.5 text-[11px] font-bold text-gray-400 uppercase tracking-wider border-b border-gray-200 bg-gray-50">
          <span>Ticket</span><span>Client</span><span>Priority</span><span>Status</span><span>Assigned To</span>
        </div>
        {loading ? (
          <div className="p-4 flex flex-col gap-2">{[...Array(4)].map((_,i) => <div key={i} className="h-14 bg-gray-100 rounded animate-pulse" />)}</div>
        ) : tickets.length === 0 ? (
          <div className="flex flex-col items-center py-16 text-gray-400 gap-2"><span className="text-3xl">✅</span><p className="text-sm font-medium">No escalated tickets — all clear!</p></div>
        ) : (
          tickets.map(t => (
            <div key={t.id} className="grid grid-cols-[1fr_130px_90px_110px_120px] items-center px-5 py-4 border-b border-gray-100 hover:bg-red-50/30 transition-colors">
              ...
              <span className="text-xs text-gray-600 truncate">{t.assignee?.full_name || <em className="text-gray-400">Unassigned</em>}</span>
            </div>
          ))
        )}
      </div>
