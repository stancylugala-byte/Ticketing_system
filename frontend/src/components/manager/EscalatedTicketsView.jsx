  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between">
        ...
        <span className="px-3 py-1.5 bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/30 text-red-700 dark:text-red-400 text-xs font-bold rounded-lg">
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm overflow-hidden">
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center py-16 text-gray-400 dark:text-slate-500 gap-2">
            ...
            <p className="text-sm font-medium">No escalated tickets — all clear!</p>
          </div>
        ) : (
          tickets.map(t => (
            <div key={t.id} className="grid grid-cols-[1fr_130px_90px_110px_120px] items-center px-5 py-4 border-b border-gray-100 dark:border-slate-700 hover:bg-red-50/30 dark:hover:bg-red-500/5 transition-colors">
              ...
              <span className="text-xs text-gray-600 dark:text-slate-300 truncate">{t.assignee?.full_name || <em className="text-gray-400 dark:text-slate-500 not-italic">Unassigned</em>}</span>
            </div>
          ))
        )}
      </div>
