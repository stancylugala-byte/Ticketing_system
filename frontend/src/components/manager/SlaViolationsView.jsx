  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between">
        ...
        {violations.length > 0 && (
          <span className="px-3 py-1.5 bg-red-100 dark:bg-red-500/20 border border-red-300 dark:border-red-500/40 text-red-700 dark:text-red-400 text-xs font-bold rounded-lg animate-pulse">
      ...
      {violations.length === 0 && !loading ? (
        ...
        <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm overflow-hidden">
          ...
          {loading ? (
            ...
            violations.map(v => (
              <div key={v.id} className="grid grid-cols-[1fr_120px_90px_110px_100px_120px] items-center px-5 py-4 border-b border-gray-100 dark:border-slate-700 bg-red-50/20 dark:bg-red-500/5 hover:bg-red-50/40 dark:hover:bg-red-500/10 transition-colors">
                <div className="flex flex-col gap-0.5 min-w-0 pr-3">
                  <span className="text-[11px] font-bold text-blue-600 dark:text-blue-400">#{v.id.slice(0,8).toUpperCase()}</span>
                ...
                <span className="text-xs text-gray-600 dark:text-slate-300 truncate">{v.assignee?.full_name || <em className="text-gray-400 dark:text-slate-500 not-italic">None</em>}</span>
                <span className="text-xs font-bold text-red-600 dark:text-red-400">+{v.overdueSince}h overdue</span>
              </div>
            ))
          )}
        </div>
