  ...
  return (
    <div className="flex flex-col gap-4">
      <div>
        ...
        <p className="text-sm text-gray-500">Active ticket load per support officer — prevents burnout</p>
      </div>

      <div className="grid grid-cols-2 gap-4">
        {/* Table */}
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
          <div className="px-5 py-3 border-b border-gray-200 bg-gray-50 grid grid-cols-[1fr_60px_100px_80px] text-[11px] font-bold text-gray-400 uppercase tracking-wider">
          ...
          {loading ? (
            ...
            data.map(d => (
              <div key={d.id} className="grid grid-cols-[1fr_60px_100px_80px] items-center px-5 py-3.5 border-b border-gray-100 hover:bg-gray-50 transition-colors">
                ...
                <span className="text-[10px] font-semibold text-gray-500">{d.role === 'SupportOfficer' ? 'Officer' : 'Dev'}</span>
                <LoadIndicator value={d.open} max={maxOpen} />
                <span className="text-sm text-gray-600">{d.pending}</span>
              </div>
            ))
          )}
        </div>
        ...
        <div className="flex flex-col gap-3">
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
            <h3 className="text-sm font-bold text-gray-700 mb-4">Load Summary</h3>
            <div className="flex flex-col gap-3">
              {data.map(d => {
                ...
                return (
                  <div key={d.id} className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm text-gray-700">{d.full_name}</span>
                    </div>
